                    ...
                    <tr>
                        ...
                        <td><div class="" style="display: flex; justify-content:center; gap:10px " >
                            ...
                            <div class="education__item-details">
                                <form action="{{route('expeirence.update', ['id' => $item->id])}}" method="post">
                                    @csrf 
                                    @method('put')  
                                <h3 class="education__title">{{$item->title}}</h3>
                                <div class="input__education">
                                    <input type="text" class="input_educations" name="title" placeholder="{{$item->title}}"  value="{{$item->title}}">
                                    
                                    <span>name</span>
                                </div>
                                <h3 class="education__title">{{$item->role}}</h3>
                                <div class="input__education">
                            ...
                            <form action="{{route('expeirence.destroy', ['id' => $item->id])}}" method="post">
                                @csrf 
                                @method('delete')  
...
<div class="educations__modal">
    <div class="educations__modal-content">
        ...
        <h3 class="services__modal-title">Add Expeirence</h3>
        
        <div class="contact__content">
            <form action="{{ route('expeirence.add')}}" class="contact-form" method="post">
                @csrf 
                @method('post')  
                <h3 class="services__modal-title">Work Place</h3>
                <div class="input__container">
                    <input type="text" class="input" name="title">
                    <span>Work Place</span>
                </div>

                <h3 class="services__modal-title">Work Role</h3>
                <div class="input__container">
